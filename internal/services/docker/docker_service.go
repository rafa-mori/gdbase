...
func newDockerServiceBus(logger *logz.LoggerZ) (ci.IDockerService, error) {
	...
	dockerService := &DockerService{
		Logger:     logger,
		reference:  types.NewReference("DockerService").GetReference(),
...
func (d *DockerService) InitializeWithConfig(ctx context.Context, dbConfig *kbx.RootConfig) error {
	if dbConfig != nil && kbx.DefaultTrue(dbConfig.Enabled) {
		...
		logz.Log("success", "Database services setup completed successfully")
	} else if dbConfig == nil {
		logz.Log("warn", "Database configuration is nil... skipping database services setup")
		return nil
	}
	logz.Log("debug", "Database settings stored in DockerService properties")
...
func (d *DockerService) StartContainer(serviceName, image string, envVars map[string]string, portBindings map[nat.Port]struct{}, volumes map[string]struct{}) error {
	if !IsDockerRunning() {
		logz.Log("fatal", "Docker is not running. Please start Docker and try again.")
	...
	if err != nil {
		logz.Log("error", fmt.Sprintf("Error pulling image: %v", err))
	...
	for volume := range volumes {
		...
		if err != nil {
			logz.Log("error", fmt.Sprintf("Error getting structured volume: %v", err))
...
func (d *DockerService) CreateVolume(volumeName, pathsForBind string) error {
	...
	for _, vol := range volumes.Volumes {
		if vol.Name == volumeName {
			logz.Log("debug", fmt.Sprintf("Volume %s already exists, skipping creation", volumeName))
			return nil
		}
	}
	...
	if filepath.IsAbs(structuredVolume.HostPath) {
		...
		logz.Log("info", fmt.Sprintf("Volume %s created at %s", vol.Name, structuredVolume.HostPath))
	}
...
func (d *DockerService) GetVolumesList() ([]*v.Volume, error) {
	...
	if err != nil {
		logz.Log("fatal", fmt.Sprintf("Error listing volumes: %v", err))
	}
